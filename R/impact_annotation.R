...
impact_annotation = function(i, bed) {
  ...
  genes_overlapped <- hg38_gencode_genelocations %&% sub_gr
  
  bp1_gene <- hg38_gencode_genelocations %&% bp1_gr
  bp2_gene <- hg38_gencode_genelocations %&% bp2_gr
  ...
  if(bp1_bp2_gene_count > 0) {
    exons_bp1_gene <-hg38_ensembl_exonlocations %&% bp1_gene
    exons_overlapped_bp1 <- exons_bp1_gene %&% sub_gr
    
    exons_bp2_gene <-hg38_ensembl_exonlocations %&% bp2_gene
    ...
    # if the breakpoint impacts exons in a gene it has a cooding effect
    ...
    ### if it impacts no exons and spans multiple genes
    if(exons_impacted_in_bp_genes == 0 & length(genes_overlapped) > 0 & sub$svtype %in% c('h2hINV', 't2tINV','INV')) {
      sub <- cbind(sub, funct_annot = "INV")
      return(sub)
    }
    if(exons_impacted_in_bp_genes == 0 & length(genes_overlapped) > 0 & sub$svtype %in% c('DEL','DUP')) {
      sub <- cbind(sub, funct_annot = "CN")
      return(sub)
    }
  } else {
...
funct_annot = function(bp = NULL, cores = 1) {
  if(is.null(bp)) {
    stop('NULL input')
  }
  cat("Annotating SVs...")
